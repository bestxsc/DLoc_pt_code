...
def train(model, train_loader, test_loader):
    ...
    for epoch in range(model.opt.starting_epoch_count + 1, model.opt.n_epochs + 1):  # opt.niter + opt.niter_decay + 1):
        ...
        for i, data in enumerate(train_loader):
            ...
            error.extend(
                localization_error(dec_outputs.data.cpu().numpy(), data[output_index].cpu().numpy(), scale=0.1))

            write_log([str(model.decoder.loss.item())], model.decoder.model_name, log_dir=model.decoder.opt.log_dir,
                      log_type='loss')
            if opt_exp.n_decoders == 2:
                write_log([str(model.offset_decoder.loss.item())], model.offset_decoder.model_name,
                          log_dir=model.offset_decoder.opt.log_dir, log_type='offset_loss')
        ...
        if opt_exp.n_decoders == 2:
            epoch_offset_loss /= i
        write_log([str(epoch_loss)], model.decoder.model_name, log_dir=model.decoder.opt.log_dir,
                  log_type='epoch_decoder_loss')
        if opt_exp.n_decoders == 2:
            write_log([str(epoch_offset_loss)], model.offset_decoder.model_name,
                      log_dir=model.offset_decoder.opt.log_dir, log_type='epoch_offset_decoder_loss')
        write_log([str(median_error_tr)], model.decoder.model_name, log_dir=model.decoder.opt.log_dir,
                  log_type='train_median_error')
        write_log([str(error_90th_tr)], model.decoder.model_name, log_dir=model.decoder.opt.log_dir,
                  log_type='train_90th_error')
        write_log([str(error_99th_tr)], model.decoder.model_name, log_dir=model.decoder.opt.log_dir,
                  log_type='train_99th_error')
        write_log([str(nighty_percentile_error_tr)], model.decoder.model_name, log_dir=model.decoder.opt.log_dir,
                  log_type='train_90_error')
        if (epoch == 1):
...
def test(model, test_loader, save_output=True, save_name="decoder_test_result", save_dir="", log=True):
    ...
    if log:
        write_log([str(median_error)], model.decoder.model_name, log_dir=model.opt.log_dir,
                  log_type='test_median_error')
        write_log([str(nighty_percentile_error)], model.decoder.model_name, log_dir=model.opt.log_dir,
                  log_type='test_90_error')
        ...
        if opt_exp.n_decoders == 2:
            write_log([str(total_offset_loss)], model.decoder.model_name, log_dir=model.opt.log_dir,
